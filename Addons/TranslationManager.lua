...
local function fetchTranslation(text, from, to, callback)
    ...
    task.spawn(function()
        ...
        if ok and raw then
            ...
            if jok and data and data.responseData and data.responseData.translatedText then
                local t = data.responseData.translatedText
                if not t:find("MYMEMORY WARNING") and #t > 0 then
                    result = t
                end
            end
        end
...
function TranslationManager:SetLanguage(code)
    self.Language = code
    self:SaveSettings()
    for originalText, callbacks in pairs(self.Registered) do
...
function TranslationManager:TranslateWindow(window)
    ...
    local function trySet(obj, setter, text)
        self:Register(text, function(translated)
            pcall(function() obj[setter](obj, translated) end)
        end)
    end
